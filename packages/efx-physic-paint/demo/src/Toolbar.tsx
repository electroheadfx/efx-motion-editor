...
export function Toolbar({ engine, onPlay, onStop, isPlaying, animFrame, animTotal, onSettingsChange, onError }: Props) {
  ...
  const onSize = useCallback((v: number) => { setSize(v); engine.setBrushSize(v) }, [engine])
  const onOpacity = useCallback((v: number) => { setOpacity(v); engine.setBrushOpacity(v) }, [engine])
  const onDetail = useCallback((v: number) => { setDetail(v); engine.setEdgeDetail(v) }, [engine])
  const onAntiAlias = useCallback((v: number) => { setAntiAlias(v); engine.setAntiAlias(v) }, [engine])
  const onPickup = useCallback((v: number) => { setPickup(v); engine.setPickup(v) }, [engine])
  const onEraseStr = useCallback((v: number) => { setEraseStr(v); engine.setEraseStrength(v) }, [engine])
  ...
  const onColor = useCallback((e: Event) => {
  ...
  const onGrainStr = useCallback((v: number) => {
    setGrainStr(v)
    engine.setEmbossStrength(v)
  }, [engine])
