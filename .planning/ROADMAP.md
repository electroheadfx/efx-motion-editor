...
- [x] **Phase 43.3: Motion and Static Group Drag Within Free Space** — Group Rail drag moves only the selected Motion/Static Group inside free space with ghost/clamp semantics, persistent vacated gaps, and stable selection/cursor (completed 2026-08-13)
...
- [x] 43.5-02-PLAN.md — ToolCase toolbox popover relocation + keyboard routing refinement (Interpolation + Key Spacing, live Blend badge)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 43.5-03-PLAN.md — push prepare/commit pair + product-reason mapper + atomic one-Undo/one-Redo history
- [x] 43.5-04-PLAN.md — usePhysicsPaintPushDrag armed gesture session hook (new, port-injected)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 43.5-05-PLAN.md — strip wiring: Push pill, armed lifecycle/disarm rules, hover pre-highlight + straddle preflight, ghost/gap/blocked-edge preview paint

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 43.5-06-PLAN.md — automated gate freeze + native visual UAT (13 prompt steps + UI-SPEC matrix, fully restarted frozen session)
...
- [x] 43.6-08-PLAN.md — Selection bridging: seedRailSetSelection carries a plain-selected rail into the rail set as anchor on the first modifier gesture (RC-B/RC-D, spec M1), synchronous Loop Clip modifier commit so Delete right after routes to Delete Rails (G-43.6-1/3/4/8)
- [x] 43.6-09-PLAN.md — Solo armed visual: add the .physics-paint-push-tool-button base class to the Solo template so the compound selector renders the orange armed tint, blocking human-verify (RC-C; G-43.6-2/7)
