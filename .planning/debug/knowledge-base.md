...
## studio-reopen-empty-boot — display-only docSync changes deduped by the bare revision, then a reference-only launch carrier bricked the Studio boot

- **Date:** 2026-09-21
- **Error patterns:** empty Studio reopen, Engine not ready, Tracks 0, black canvas after Studio close, photo reference hide show, display preference lost, visibleInStudio, opacity, transform, docSync push skipped, register guard dedupe, reference-only record on a runtime path, carries no inline raster bytes, createPhysicPaintLaunchContext, launch pack bytes, materializeRuntimeRotoMediaBytes, recordsAsRuntimeFrames throw in setter, mirrorRotoPhysicalDocument, canonical revision vs sync fingerprint
- **Root cause(s):** TWO, one chain. (A) Display persistence: the docSync change detection on BOTH sides — the child's push guard and the parent's register guard — compared `buildEfxPaintDocumentRevision`, which deliberately excludes the photo-reference display fields (visibleInStudio / opacity / transform / transformLocked; D-07 vs D-11/D-12/D-13). A display-only change compared "equal": deduped child-side (never pushed) and skipped parent-side (never registered), so the choice never reached the saved document. (B) Empty boot: once (A) let display changes register, the parent's receive mirror (`mirrorRotoPhysicalDocument`) replaced its runtime records with the pushed persisted-shape (reference-only) ones, and `createPhysicPaintLaunchContext` packed that runtime WITHOUT materializing bytes — so the launch carrier shipped a media-only record whose bytes the child's strict launch seed (`recordsAsRuntimeFrames` → `requirePhysicPaintRotoInlineBytes`) asserted on, throwing inside the synchronous preact launch-context setter; the launch replacement handoff aborted and the Studio froze at its pre-launch shell until an app restart (which re-materializes at project open).
- **Fix:** (A) new `buildEfxPaintDocumentSyncFingerprint` (efxPaintDocumentRevision.ts) = canonical revision + the photo display term; both guards compare it (the canonical revision keeps its deliberate exclusion). (B) `physicPaintStore.materializeRuntimeRotoMediaBytes(layerId)` re-attaches bytes before the pack is built (bridged `_frameMediaBytes` first with no IO, then `_packageDirProvider` + the digest-verified `resolveFrameMediaBytes`; per-key failures logged loudly, never blocking a launch) and `recordsAsRuntimeFramesToleratingReferences` seeds the four Studio boot SEED sites; `requirePhysicPaintRotoInlineBytes` and `recordsAsRuntimeFrames` keep their strict publish contracts. Commit `d5437a15`.
- **Files changed:** app/src/efx-paint/document/efxPaintDocumentRevision.ts, app/src/components/physic-paint/bridge/documentSyncPushGuard.ts, app/src/components/physic-paint/bridge/documentSyncPushGuard.test.ts, app/src/lib/physicPaintBridge.ts, app/src/stores/physicPaintStore.ts, app/src/lib/efxPaintMediaMaterialize.ts, app/src/components/physic-paint/hooks/useRotoFramePersistenceCoordinator.ts, app/src/components/physic-paint/PhysicsPaintStudio.tsx, app/src/lib/physicPaintBridge.launchPackBytes.test.ts, app/src/components/physic-paint/PhysicsPaintStudio.test.ts
- **Why not caught:** No gate existed for either class. (A) the guard tests only exercised changes that DID bump the revision (background fallback), so the display-only class had no assertion, and tsc cannot (both compared values are strings). (B) the vitest harness cannot mount the Studio (Tauri window + engine deps) and drives the non-Tauri DOM fallback, so neither the parent's receive mirror nor the child's synchronous launch seed executed in any test; the child's throw was swallowed by the coordinator's catch (console-caught, never uncaught) — the same harness blindness that falsified the earlier e21 and ffh rounds.
- **Recurrence guard:** Regression test `app/src/lib/physicPaintBridge.launchPackBytes.test.ts` — LEG 1/1b/3 drive the REAL child→parent chain (real transport + real installed listener + real launch door) and are RED at base with the production throw; LEG 4 pins the parent register for a display-only push (RED at base with the bare-revision guard). Guard-level pin: the display suite in `app/src/components/physic-paint/bridge/documentSyncPushGuard.test.ts` (RED at base, control included). Both laws now sit side by side in `efxPaintDocumentRevision.ts` with doc comments stating the split. Plus this KB pattern for Phase-0 recall on "empty Studio reopen / Engine not ready after close".
---
