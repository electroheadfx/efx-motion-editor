...
- [x] 36.10-03-PLAN.md — Surface background-only/transparent missing-frame state through existing Roto workflow strip and session boundaries.

**Wave 4** *(blocked on Wave 3 UI/session wiring)*

- [x] 36.10-04-PLAN.md — Prove preview/export parity, real-key adjacency, and alpha-separation regressions.

**Wave 5** *(blocked on Wave 4 automated gates)*

- [x] 36.10-05-PLAN.md — Record automated validation and run user-owned live preview/export UAT.
...
- [x] 36.11-02-PLAN.md — Harden cached-base clean/no-change semantics and live-overlay-only Clear behavior.
...
- [x] 36.12-01-PLAN.md — Define and test integer whole-layer interpolation spans.

**Wave 2** *(blocked on Wave 1 span contract)*

- [x] 36.12-02-PLAN.md — Generate alpha-only renderable cache frames with real-key authority and stale regeneration.
- [x] 36.12-03-PLAN.md — Expose preview-only generated selection and minimal Interpolation toggle UI.
...
Use `SPECS/36.x-phases/phase-36.13-dynamic-interpolation-spacing/spec-36.13-dynamic-interpolation-spacing.md` as the source of truth for this phase.

**Goal:** As a stop-motion animator, I want intentionally distant real Roto keys to create remembered per-segment interpolation spacing, so that custom timing survives global in-between count changes, interpolation toggles, save/load, preview, and export.
**Mode:** mvp
...
- Real Roto keys remain authoritative; generated interpolation frames remain derived/render-only.
- Adjacent source real keys use the global in-between count unless a segment has an explicit spacing override.
- A real key intentionally created farther away than the global spacing records custom per-segment spacing and must not be collapsed by later global count changes.
- The source/display model must cover hydration, parent bridge sync, project save/load, close/reopen, preview, and export.
- Keep this as an MVP behavior correction before the final Pencil timeline UI pass.

Plans:

**Wave 1**

- [x] 36.13-01-PLAN.md — Define and test override-aware source/display spacing contracts.
...
- [x] 36.13-03-PLAN.md — Rebase, drop, and create custom spacing through Roto key utility transactions.

**Wave 4** *(blocked on Waves 1-3 model/store/transaction support)*

- [x] 36.13-04-PLAN.md — Wire far-empty save/paste and compact custom-span feedback into Studio and workflow strip.

**Wave 5** *(blocked on Waves 2 and 4 generated cache/UI wiring)*

- [x] 36.13-05-PLAN.md — Prove onion-skin real-key anchors and preview/export parity for custom spans.
...
- Generated cells remain render-only but are valid physical drag destinations. Under D-29, occupied before/after boundaries remove only the moved identity and ripple at the destination: from `A@1,B@3,C@5,D@8`, before D yields `A@1,C@5,B@8,D@9` and after D yields `A@1,C@5,D@8,B@9`; whole-cell frame 6 remains the source-closing `A@1,C@4,B@6,D@8` result.
...
- [x] 36.14-01-PLAN.md — Establish the production-only pre-UAT boundary; replace the durable Roto timing schema with stable `keyId` plus direct physical `appFrame`, persisted interpolation enabled state, and a separate Script Motion contract; remove compatibility/migration paths (no regression files touched until 36.14-12 approval).

**Wave 2** *(blocked on Wave 1)*

- [x] 36.14-02-PLAN.md — Implement the pure physical timeline resolver (ordering, exact interior derived cells, Insert/Delete/Drag/Force Spacing proposal resolution, uniqueness/range/capacity validation, deterministic selection/presentation metadata) used by preview and commit.

**Wave 3** *(blocked on Wave 2)*

- [x] 36.14-03-PLAN.md — Cut store, hydration, layer initialization, selection, and shared workflow callers over to physical records and runtime-derived cells.

**Wave 4** *(blocked on Wave 3)*

- [x] 36.14-04-PLAN.md — Build the single acknowledged physical-edit coordinator with pending-operation serialization, revalidation, immutable snapshots, optimistic publication, exact acknowledgement matching, and fail-closed restoration.

**Wave 5** *(blocked on Wave 4)*

- [x] 36.14-05-PLAN.md — Route accepted-only physical history, Undo, and Redo through the shared coordinator.

**Wave 6** *(blocked on Wave 5)*

- [x] 36.14-06-PLAN.md — Deliver ripple Insert Frame and Delete Frame through the pure resolver and acknowledged coordinator.

**Wave 7** *(blocked on Wave 6)*

- [x] 36.14-07-PLAN.md — Deliver single-key ripple Drag as complete cut-and-insert proposals for empty/generated destinations and occupied before/after boundaries.

**Wave 8** *(blocked on Wave 7)*

- [x] 36.14-08-PLAN.md — Deliver Force Spacing through the same resolver and coordinator for every nonnegative integer `N`.
...
- [x] 36.14-10-PLAN.md — Migrate remaining cache/live-pixel/navigation/visual ownership and production importers out of `useRotoPersistenceIntegration.ts`, then cut playback, onion/reference, preview, export, missing/background rendering, and timeline-length consumers over to direct physical positions.
...
- [x] 36.14-22-PLAN.md — Make enabled-only canonical physical interpolation the sole acknowledged control authority and restore visible first-click state.
...
- [x] 36.14-26-PLAN.md — Publish Play Script output through the canonical physical-map coordinator with stable identities and durable PNG payloads.

**Wave 25** *(gap closure; blocked on Plans 21-27)*

- [x] 36.14-28-PLAN.md — Perform D-29/D-30 integrated bounded static read-only review and hand off readiness for user-owned native gap UAT.
...
- [x] 36.15-01-PLAN.md — Styled tooltip component + icon-only guarded bottom action row (wave 1)
- [x] 36.15-02-PLAN.md — EFX Motion layer real-Roto-key diamond markers (wave 1)
- [x] 36.15-03-PLAN.md — Application selection guard in app/index.html (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 36.15-04-PLAN.md — Header pill-island recomposition + close button (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 36.15-05-PLAN.md — Status capsule + status stack/legend removal + per-cell tooltips (wave 3)
...
- [x] 36.15-07-PLAN.md — Discard Script relocation to the Scripts toolbar (wave 5)
...
- [x] 38-07-PLAN.md — Post-UAT regression tests: group seam (resolver, multi-selection aftermath, group clipboard shape). `vitest run` only.
- [x] 38-08-PLAN.md — Post-UAT regression tests: presentation + final gate (capsule idle-context assertions, tooltip placement describe, full vitest run + typecheck + build green).
...
- [x] 38.1-01-PLAN.md — Timeline-model structural/frame split (useRotoTimelineModel.ts:77 defect) + D-07 instrumentation test: zero structural rebuilds on frame writes, byte-compatible model surface.
- [x] 38.1-02-PLAN.md — rAF UI flush scheduler (D-04) + latest-wins navigation generation tokens (D-05): pure, test-locked utilities.
...
- [x] 38.1-04-PLAN.md — Strip cheap render: fine-grained per-cell derivation, rotoDragValidityKey structural memo, byte-identical DOM contract (D-09/D-10).
...
**Reopened Wave 5** *(complete; native-approved 2026-07-28)*
...
- [x] 38.1-13-PLAN.md — Route cached playback through the canonical transparent/background texture/paper grain compositor with behavior-first coverage.
- [x] 38.1-14-PLAN.md — Finalize and replay physics sessions using the completed mode before restoring the prior engine mode.
- [x] 38.1-15-PLAN.md — Treat capacity as physical timeline structural identity and publish capacity-only replacements.
- [x] 38.1-16-PLAN.md — Make optional profiling fail closed when localStorage access or reads throw.

**Gap Closure Wave 10** *(blocked on Plan 13 completion)*

- [x] 38.1-17-PLAN.md — Add bounded Node-runtime CanvasMount hook/effect tests for stable-forwarder freshness, dimension dependencies, observer/rAF cleanup, and idempotent unmount; retain memo/key source assertions as architecture checks only.
