...
| `app/src/components/physic-paint/physicsPaintRotoKeyController.ts` | utility/controller | pure transaction transform | `app/src/components/physic-paint/physicsPaintWorkflowState.ts` | closest pure-helper seam |
| `app/src/components/physic-paint/physicsPaintRotoKeyController.test.ts` | test | pure transaction regression | `app/src/components/physic-paint/physicsPaintWorkflowState.test.ts` | closest helper-test seam |
...
### `app/src/components/physic-paint/physicsPaintRotoKeyController.ts` (utility/controller, pure transaction transform)

**Closest analog:** `app/src/components/physic-paint/physicsPaintWorkflowState.ts`

**Classification:** New Plan 05 file. Treat as the Roto key utility controller/store boundary beside the workflow-state helpers, not as a component, bridge, or engine adapter.

**Apply these analog patterns:**
- Export named operation/result types beside named pure functions, matching the `RotoDuplicateKeyResult`/`RotoInsertKeyResult`/`RotoDeleteKeyResult` pattern above.
- Export a derived action-state API such as `deriveRotoKeyUtilityActionState(...)` for Copy/Duplicate/Insert/Delete/Paste eligibility, copied-key availability, disabled reasons, dirty-save-before-action intent, and in-flight inputs.
- Normalize frame numbers deterministically and keep transaction output sorted/stable, following the existing `normalizeRealRotoKeyFrames` helper style.
- Return deterministic transaction data plus cleanup decisions for generated targets, deleted frames, editable/local preview maps, and active restore intent.
- If state must live outside `PhysicsPaintStudio.tsx`, use Preact-native reactivity, preferably `@preact/signals` (`signal`/`computed`), and keep the API testable without component effects.
- If the boundary remains pure/stateless, keep the file free of Preact imports, engine imports, DOM access, and `physicPaintStore` singleton access; accept cache, copied-key, blank-frame factory, editable/local preview map data as arguments and document the pure/stateless choice in `36.7-05-SUMMARY.md`.

**Controller/store responsibility boundary:** The boundary owns key utility state/cache decisions: source eligibility, paste target eligibility, copied-key action state, dirty-save-before-action decision, transaction frame layout, generated-target cleanup, deleted-frame cleanup, and active restore intent for Copy/Duplicate/Insert/Delete/Paste. It does not persist to the bridge/store directly unless a small dedicated Signals-backed store is created for moved-out state. Studio remains the adapter that links boundary output to `physicPaintStore`/bridge persistence, launch-context refresh, engine canvas restore, status messages, and workflow-strip props.

**Planner note:** This file exists to centralize Plan 05 cache/state coherence logic after UAT exposed scattered Studio mutation gaps. It should remain narrowly testable like `physicsPaintWorkflowState.ts` or explicitly Signals-backed if it owns external state; any new broad `useEffect` or scattered ref/useState orchestration in `PhysicsPaintStudio.tsx` violates the intended pattern.
...
### `app/src/components/physic-paint/physicsPaintRotoKeyController.test.ts` (test, pure transaction regression)

**Closest analog:** `app/src/components/physic-paint/physicsPaintWorkflowState.test.ts`

**Classification:** New Plan 05 regression test file. Use the existing Vitest helper-test style: small in-memory fixtures, direct pure-function assertions, no browser/server harness, and no one-off test config.

**Apply these analog patterns:**
- Import `describe`, `expect`, and `it` from `vitest`.
- Import the controller functions/types from `./physicsPaintRotoKeyController` and `PhysicPaintRotoCacheFrame` or related types from `../../types/physicPaint` as needed.
- Group cases under a focused `describe('physicsPaintRotoKeyController', ...)` block.
- Use distinctive `dataUrl` fixture values so assertions can prove stale generated/reference payloads are absent after Insert/Delete/Paste.
- Assert full transaction objects or stable subsets with explicit frame arrays, removed/changed frames, active restore intent, and real-key frame payload sources.

**Planner note:** This test file proves Plan 05 behavior before Studio integration. It should not read TSX source or assert UI markup; source-contract assertions remain in `PhysicsPaintWorkflowStrip.test.ts`. It should prove derived action state/transaction outputs cover Copy, Duplicate, Insert, Delete, Paste, dirty-save-before-action, generated-target cleanup, deleted-frame cleanup, and restore intent.
...
**Planner note:** `PhysicsPaintStudio.tsx` must keep an adapter role, not the owner of key utility state/cache truth. It may snapshot current engine state, call the existing `flushRotoFrame(..., { force: true })` boundary when the controller/store says dirty-save-before-action is required, persist controller transaction frames through the existing bridge/store path, refresh launch-context cache from `physicPaintStore`, restore/clear the active canvas from boundary restore intent, and forward unchanged workflow-strip props. Do not add broad `useEffect` branches or scattered local `useState`/ref orchestration for key utility cache, generated-target cleanup, deleted-frame cleanup, dirty-save-before-action, or restore intent. Paste must not call `requireCurrentRealRotoKey()` for the target; it only requires a copied key and a valid target frame.
...
**Apply to:** Studio cache updates and any store helper added for shifting keys. Do not apply direct store access to `physicsPaintRotoKeyController.ts`; the controller returns transaction data and Studio performs bridge/store persistence.
```typescript
...
None. All planned files have exact existing seams, closest pure-helper analogs, and/or spec visual references. Plan 05's new controller/test files use `physicsPaintWorkflowState.ts` and `physicsPaintWorkflowState.test.ts` as closest analogs rather than exact existing seams.
...
**Files scanned:** 14 candidate files plus project instructions/skills; Plan 05 adds 2 new files classified by closest analog  
