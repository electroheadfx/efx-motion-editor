...
**Classification:** New Plan 05 file. Treat as a pure controller/utility beside the workflow-state helpers, not as a component, store, bridge, or engine adapter.
...
- Accept all cache, copied-key, blank-frame factory, editable/local preview map data as arguments and return deterministic transaction data.
- Keep the file free of Preact imports, engine imports, DOM access, and `physicPaintStore` singleton access.

**Controller responsibility boundary:** The controller decides cache transaction shape and active-frame restore intent for Duplicate/Insert/Delete/Paste. It does not persist to the bridge/store directly. Studio is the integration layer that links controller output to `physicPaintStore`/bridge persistence, launch-context refresh, engine canvas restore, status messages, and workflow-strip props.

**Planner note:** This file exists to centralize Plan 05 cache/state coherence logic after UAT exposed scattered Studio mutation gaps. It should remain narrowly testable like `physicsPaintWorkflowState.ts`; any direct store mutation in this pure controller violates the intended pattern.
...
**Planner note:** This test file proves Plan 05 behavior before Studio integration. It should not read TSX source or assert UI markup; source-contract assertions remain in `PhysicsPaintWorkflowStrip.test.ts`.
...
**Planner note:** Add a shared async key-action runner in this file, copying the save-before-navigation ordering from `requestRotoFrameNavigation`: snapshot current source, if dirty save via `flushRotoFrame(..., { force: true })`, cancel on failure and keep dirty frame visible, then ask `physicsPaintRotoKeyController.ts` for a pure transaction. Studio, not the pure controller, persists the transaction through the existing bridge/store path, refreshes launch-context cache from `physicPaintStore`, restores/clears the active canvas, and forwards unchanged workflow-strip props. Paste must not call `requireCurrentRealRotoKey()` for the target; it only requires a copied key and a valid target frame.
