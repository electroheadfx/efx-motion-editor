...
- **D-04:** Application options live in the expanded Play Script confirmation dialog (mode, override color, Script Motion, Hold Loop controls); the Scripts panel gains a compact read-only summary of the current options. ~~The dialog remains mounted directly in the Studio grid with isolated full-height styles~~ (SUPERSEDED 2026-08-06 → D-19: the Play Script dialog is a compact modal overlay above the Paint interface, never occupying the canvas grid cell; the Phase 36.14 constrained-dark-pane regression lesson still applies to any other Studio-pane surface).
...
- **D-16 (REVISED 2026-08-06, final):** The dialog adopts the proposal's compact dark modal visual direction (Image #153 = approved target; Image #154 = rejected light full-canvas implementation). Layout rows: **Row 1** compact header (`Play Script` left, `Max {N} · F{start}–F{end}` right); **Row 2** full-width compact Mode card (segmented control + dynamic helper line); **Row 3** two-column main-options grid — LEFT: one `Timing` card occupying the full row height (Frames / Frames per cycle + helper/validation, Repeat + Infinity + helper/validation); RIGHT: a stacked two-card column with `Color` on top and `Motion wiggle` directly underneath, whose combined height + internal gap EQUALS the Timing card height (individual heights may differ; slight shared row-height growth is allowed to preserve equality without scrolling); **Row 4** full-width Requested/Effective summary bar; **Row 5** compact footer INSIDE the modal (progress/status + Cancel + Generate). Motion wiggle is NEVER a separate full-width row and NEVER above Color. The proposal's surrounding demo shell remains out of scope (D-17).
- **D-19:** The Play Script dialog is a compact independent MODAL OVERLAY (supersedes the D-04 Studio-grid full-height mounting and the reused light dialog tokens for this dialog): it opens above the existing Paint interface via the sole Scripts-panel toolbar trigger (D-17), leaves the canvas and Studio layout in place behind it, never replaces or occupies the canvas grid cell, never renders as a full-page/full-height editor surface, never resizes the Paint layout, and closes via Cancel/Escape/successful completion without changing the Paint layout. It uses its own dialog-scoped DARK token set derived verbatim from the proposal (`--surface/--raised/--inset/--foot/--fg/--muted/--faint/--border/--accent/--accent-hi/--ok/--radius`, oklch values locked in UI-SPEC) — the old light Play Script palette is rejected. All selectors remain scoped to the Play Script modal so surrounding Paint UI is not restyled. **Maximum compactness with NO scrolling:** compact header (15px title), compact cards/padding/gaps per the proposal metrics; the complete dialog must fit at the minimum supported application window size with all rows visible — no vertical or horizontal scrollbar, no body scrolling region, no clipped card/helper/summary/footer. The compact layout, not overflow scrolling, solves the height constraint (this supersedes the earlier viewport-bounded body-scroll contract for this dialog).
