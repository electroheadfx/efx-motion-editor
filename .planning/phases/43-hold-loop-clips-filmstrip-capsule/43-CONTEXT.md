...
Requirements: HOLD-01..06 (REQUIREMENTS.md). Source spec: `SPECS/milestone-v0.9.0-plan.md` §"Phase 4 — Deterministic static/hold rendering".
...
- **D-20:** Capsule copy is English only: `Loop shortened by next clip`. HOLD-06's French phrase `Boucle raccourcie par le clip suivant` is SUPERSEDED (its intent — a clear truncation label — is preserved; Phase 42 D-13 deferred the language decision to this phase). The term `clip bloquant` remains prohibited in every language.
...
- **D-24:** Only three entity kinds truncate a loop: real keys (including empty real keys), another loop's canonical start, and parent end. Interpolated/generated render-only frames are derived content — they never truncate. The boundary model is purely key-based, matching the physical-frame document authority. — **Reversibility:** costly — this boundary algebra is the shared contract between the resolver, every ops interaction (D-06..D-14), and the capsule; changing it later re-opens all three.
...
- `SPECS/milestone-v0.9.0-plan.md` §"Phase 4 — Deterministic static/hold rendering" — locked HOLD-01..05 spec text, FrameLoopClip illustrative schema, Phase 4 acceptance list; §PLAY-04 for the filmstrip capsule visual description (HOLD-06 source)
- `.planning/REQUIREMENTS.md` — HOLD-01..06 requirement statements; note D-20 supersedes the French label inside HOLD-06 (English copy ships; prohibition of `clip bloquant` stands)
...
- User specified loop-loop priority semantics verbatim: B begins at its canonical start; B is NOT pushed after A; A effective end = min(A requested end, B start, parent end); canonical Requested ranges unchanged, Effective derived; same-start collisions never resolved by hidden creation-order priority (D-14).
...
- User explicitly resolved the HOLD-06 language conflict: English-only `Loop shortened by next clip`; the French phrase is superseded and must not ship; `clip bloquant` prohibited in every language (D-20).
