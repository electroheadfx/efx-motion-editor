...
- [ ] **PLAY-01**: An explicit PlayScript application mode selects `progressive` (current accumulating behavior) or `static`/`hold` (complete script stroke set materialized on every destination frame), independent of Roto interpolation and Script Motion
