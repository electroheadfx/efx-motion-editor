...
status: planning
stopped_at: "Completed quick-260921-ffh (re-diagnosis of the Studio-origin loss after e21 was falsified by native UAT; e21's re-arm was unreachable - now reachable) - native UAT pending"
last_updated: "2026-09-21T09:41:02.000Z"
...
Last activity: 2026-09-21 - Completed quick task 260921-ffh: re-diagnosis after e21 was falsified by native UAT — verdict (a) x3 at one composite link (the push decision read a render-captured guard while the failure path re-arms the REF, so e21's retry was unreachable and the caller's pre-push clear consumed the change); three BEHAVIOURAL pins RED at base, GREEN after; steps 1-5 exonerated mutation/serialize/transport/parent-apply/carrier again; caveat — the path needs a real failed emitTo, unproven; native UAT pending
...
| 260921-ffh | Re-diagnosis after e21's causal theory was falsified by native UAT (the three Studio-origin surfaces: reference selection, background keyframes, (+) tracks — still lost on a plain Studio close while strokes persist). Verdict (a) ×3 at ONE COMPOSITE link: the push decision read a guard captured at render creation (`PhysicsPaintStudio.tsx:3914`/`:3928`) while the failure path re-arms the REF (`:3987`) and `createDocumentSyncPushGuard` latches its fingerprint BEFORE the send resolves (`documentSyncPushGuard.ts:19-20`), so a retry with no render in between read the FAILED send's content as its own duplicate, returned null, and the caller — which clears `documentSyncDirty` BEFORE the push (`:4030`/`:4054`/`:4087`) — consumed the change with no send and no re-mark. e21's re-arm was correct but UNREACHABLE; the fix makes it reachable (the hoisted decision reads the slot at decision time). Probe-fidelity gaps closed: FG-1 guard-in-the-loop, FG-2 serialize write-back, FG-3 two real realms, FG-4 teardown decisions; steps 1–5 green — mutation, serialize, transport, real parent listener, reopen carrier and the store/package seam exonerated again; the teardown gate was measured (it fires). THREE BEHAVIOURAL pins RED at base for the diagnosed reason and GREEN after, plus a strokes positive control and an e21-guardrail negative control green in both runs. Full suite 4071 pass (base 4065 + 6 new legs, 0 absorbed), tsc clean, bjm 313 pass, c7x 218 pass, scope + event-pair gates clean. CAVEAT: the diagnosed path requires a real failed `emitTo` in the live app — unproven; if UAT still loses the surfaces, the next target is close/teardown DELIVERY, which no unit leg can execute. Native UAT PENDING (5 rows) | 2026-09-21 | 026e70a9 | [260921-ffh-e21-uat-falsified-reference-selection-ba](./quick/260921-ffh-e21-uat-falsified-reference-selection-ba/) |
