...
- [Phase 43.2]: Delete availability and activation now share one pure classifier across keyboard and visible controls.
- [Phase 43.2]: Group lifecycle deletion recomputes from the complete physical document after project/layer lease acquisition and publishes only after exact parent acknowledgement.
- [Phase 43.2]: Only-occurrence Delete Frame removes the complete Group while preserving delete-group-frame semantic identity.
- [Phase 43.2]: Regenerate remains inside the existing Play Script controller; lifecycle-capable Groups deepen that interface instead of introducing a second Group-operation controller.
...
- [Phase 43.2]: Referenced Action commands use a dedicated history route and never fall through to ordinary physical snapshot restoration.
...
- [Phase 43.2]: Persisted scriptId, loopId, progressive/static values, and internal Script/Loop Clip names remain unchanged; only affected product presentation migrated.
...
- [Phase ?]: One exported pure clamp function (clampPhysicPaintGroupDragDestination) is the single authority for both the resolver branch and the plan 03 ghost preview — preview-is-the-commit (D-05, RESEARCH Open Question 2)
...
- [Phase ?]: Key Rail movement uses one directional clamp authority with own-member pass-through and external real-key/Group boundaries.
- [Phase ?]: Key Rail deletion preserves survivor absolute frames and normalizes the complete successor-break collection.
- [Phase ?]: Move and delete authorize only exact current deriveKeyRailSegments membership.
...
- [Phase ?]: 43.6-08: seedRailSetSelection bridges the plain-selected single rail into the rail-set anchor at the reducer input; active set wins, malformed/null fails closed, reducer byte-untouched (M1)
...
| 260819-wzi | Fix addEmptyKey to join an existing Key Rail when destination is strictly inside a derived Key Rail segment span | 2026-08-19 | 3dd70c05 |  | [260819-wzi-fix-addemptykey-to-join-an-existing-key-](./quick/260819-wzi-fix-addemptykey-to-join-an-existing-key-/) |
...
| 260820-hq9 | addEmptyKey inside an interpolated segment: force the empty payload to own the display (paste-key reconcile) | 2026-08-20 | 076ddc67 |  | [260820-hq9-addemptykey-inside-interpolated-segment-](./quick/260820-hq9-addemptykey-inside-interpolated-segment-/) |
