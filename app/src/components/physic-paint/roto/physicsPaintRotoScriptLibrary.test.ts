...
function harness(saved = true) {
  ...
  const controller = createRotoScriptLibraryController({ request, capturePersistence: vi.fn(async () => ({ script: { provenance: { sessionId: 's', layerId: 'layer-1', sourceFrame: 4 }, sourceFrame: 4, sourceDisplayFrame: 4, sourceRevision: 1, brushes: [{ primary: { tool: 'paint', points: [{ x: 1, y: 2, p: 1, tx: 0, ty: 0, tw: 0, spd: 0 }], color: '#000000', params: { size: 1, opacity: 100, pressure: 100, waterAmount: 0, dryAmount: 0, edgeDetail: 0, pickup: 0, eraseStrength: 0, antiAlias: 0 }, timestamp: 1 }, continuations: [] }] }, scriptAlphaCanvas: {} as HTMLCanvasElement })), captureThumbnail: vi.fn(async () => ({ mimeType: 'image/webp', width: 1, height: 1, quality: 0.8, dataUrl: 'data:image/webp;base64,UklGRgQAAABXRUJQ' })), replaceClipboard: vi.fn((value) => { clipboard.current = value; return true; }), getLaunchContext: () => launch, log: vi.fn() });
...
describe('Roto script library controller', () => {
  ...
  it('drives explicit scans, stable sorting, naming and expected revisions', async () => {
    ...
    expect(test.requests.at(-1)).toMatchObject({ kind: 'rename', scriptId: 'a', expectedRevision: 'rev-a', name: 'Renamed' });
  ...
  it('rejects overlapping and stale operations after context replacement or disposal', async () => {
    ...
    const test = harness();
    test.request.mockImplementationOnce((request) => new Promise((resolve) => { settle = resolve; }));
