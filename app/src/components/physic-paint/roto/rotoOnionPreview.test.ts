...
const frame = (appFrame: number, patch: Partial<RotoOnionFrame> = {}): RotoOnionFrame => ({ frameIndex: 0, appFrame, bytes: testWebpBytes(`frame-${appFrame}`), width: 10, height: 10, ...patch });
...
describe('rotoOnionPreview', () => {
  ...
  it('filters generated/background frames, prefers dirty previews, and orders farthest first', () => {
    const result = projectRotoOnionPreviewFrames({
      ...
      launchFrames: [frame(1, { source: 'real-key' }), frame(4, { source: 'real-key', onionBytes: testWebpBytes('strokes-4') }), frame(6, { source: 'generated-interpolation' }), frame(7, { source: 'real-key', backgroundOnly: true }), frame(8, { source: 'real-key' })],
      storeFrames: [frame(2, { source: 'real-key' })],
      previewFrames: new Map([[4, frame(4, { bytes: testWebpBytes('dirty-4') })]]),
      dirtyFrames: new Set([4]),
    });
    expect(result).toEqual([
      { frame: 2, bytes: testWebpBytes('frame-2'), direction: 'previous', distance: 2, source: 'roto', kind: 'cached-composite' },
      { frame: 4, bytes: testWebpBytes('dirty-4'), direction: 'previous', distance: 1, source: 'roto', kind: 'stroke-preview' },
      { frame: 8, bytes: testWebpBytes('frame-8'), direction: 'next', distance: 1, source: 'roto', kind: 'cached-composite' },
    ]);
  });

  it('uses only surrounding real anchors for compact and distant generated displays', () => {
    const realFrames = [
      frame(0, { source: 'real-key', sourceFrame: 0, displayFrame: 0, bytes: testWebpBytes('A') }),
      frame(1, { source: 'real-key', sourceFrame: 1, displayFrame: 3, bytes: testWebpBytes('B') }),
      frame(2, { source: 'real-key', sourceFrame: 2, displayFrame: 6, bytes: testWebpBytes('C') }),
      frame(3, { source: 'real-key', sourceFrame: 3, displayFrame: 9, bytes: testWebpBytes('D') }),
      frame(14, { source: 'real-key', sourceFrame: 14, displayFrame: 14, bytes: testWebpBytes('E') }),
      frame(26, { source: 'real-key', sourceFrame: 26, displayFrame: 26, bytes: testWebpBytes('F') }),
    ];
    ...
    const anchorsAt = (currentFrame: number) => {
      ...
      }).map((item) => ({ frame: item.frame, bytes: item.bytes }));
    };

    expect(anchorsAt(3)).toEqual([{ frame: 0, bytes: testWebpBytes('A') }, { frame: 6, bytes: testWebpBytes('C') }]);
    expect(anchorsAt(1)).toEqual([{ frame: 3, bytes: testWebpBytes('B') }]);
    expect(anchorsAt(2)).toEqual([{ frame: 3, bytes: testWebpBytes('B') }]);
    expect(anchorsAt(4)).toEqual([{ frame: 0, bytes: testWebpBytes('A') }, { frame: 6, bytes: testWebpBytes('C') }]);
    expect(anchorsAt(5)).toEqual([{ frame: 0, bytes: testWebpBytes('A') }, { frame: 6, bytes: testWebpBytes('C') }]);
    expect(anchorsAt(12)).toEqual([{ frame: 6, bytes: testWebpBytes('C') }, { frame: 14, bytes: testWebpBytes('E') }]);
    expect(anchorsAt(20)).toEqual([{ frame: 9, bytes: testWebpBytes('D') }, { frame: 26, bytes: testWebpBytes('F') }]);
  });

  it('makes a generated frame traverse onions from its owning real key', () => {
    const realFrames = [
      frame(6, { source: 'real-key', sourceFrame: 6, displayFrame: 6, bytes: testWebpBytes('key-6') }),
      frame(13, { source: 'real-key', sourceFrame: 13, displayFrame: 13, bytes: testWebpBytes('key-13') }),
      frame(17, { source: 'real-key', sourceFrame: 17, displayFrame: 17, bytes: testWebpBytes('key-17') }),
    ];
  ...
  it('counts neighboring real keys rather than generated or empty display frames', () => {
    const realFrames = [
      frame(3, { source: 'real-key', sourceFrame: 3, displayFrame: 3, bytes: testWebpBytes('key-3') }),
      frame(6, { source: 'real-key', sourceFrame: 6, displayFrame: 6, bytes: testWebpBytes('key-6') }),
      frame(13, { source: 'real-key', sourceFrame: 13, displayFrame: 13, bytes: testWebpBytes('key-13') }),
      frame(17, { source: 'real-key', sourceFrame: 17, displayFrame: 17, bytes: testWebpBytes('key-17') }),
    ];
    const nonKeyPreviews = new Map(
      [7, 8, 9, 10, 11, 12, 14, 15, 16, 18, 19, 20].map((display) => [display, frame(display, { bytes: testWebpBytes(`display-${display}`) })]),
    );
  ...
  describe('loop placeholder exclusion (D-28, audit finding 6)', () => {
    const realRecord = (keyId: string, appFrame: number): PhysicPaintRotoRealKeyRecord => ({
      ...
      keyId,
      appFrame,
      payload: { frameIndex: 0, appFrame, bytes: testWebpBytes(btoa(`onion:${appFrame}`)) },
    });
