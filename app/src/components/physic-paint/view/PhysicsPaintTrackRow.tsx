 ...
 *
 * The row is presentational: it exposes a clickable header that fires
 * `onSelectTrack(trackId)` (the controller routes it through
 * `efxPaintStore.setActiveTrackId`). Cross-row mutations never originate
 * here — all edits keep routing through `studioActiveTrackId()`.
 */
...
export interface PhysicsPaintTrackRowProps {
  ...
  /** Row label shown in the row header. */
  readonly label: string;
  /** 'background' renders the fixed muted Bg row skeleton. */
  readonly kind?: PhysicsPaintTrackRowKind;
  /** The document's current active track id — the matching row gets the active treatment. */
...
/**
 * One 48px track row. The active track's row keeps the strip's rich lane
 * (rails + interactive cells) in the strip; this component renders the
 * per-track cells grid for every non-active Paint track and the fixed
