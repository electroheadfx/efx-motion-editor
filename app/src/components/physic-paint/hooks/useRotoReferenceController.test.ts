...
const frame = (appFrame: number, source: 'real-key' | 'generated-interpolation', bytes = testWebpBytes(`data:${appFrame}`)): RotoReferenceFrame => ({ appFrame, frameIndex: appFrame, source, bytes });
...
describe('Roto reference controller', () => {
  it('resolves cached frames through the exact physical cell: generated, real, dirty preview fallback, then null', () => {
    const generatedSource = {
      ...
      renderedFrame: { frameIndex: 0, appFrame: 4, bytes: pngDataUrl('generated') },
    };
    const realSource = {
      kind: 'real' as const,
      layerId: 'layer-1',
      ...
      renderedFrame: { frameIndex: 0, appFrame: 4, bytes: pngDataUrl('real') },
    };
    ...
    expect(findCachedRotoDisplayFrame(4, { getPhysicalRenderSource: () => generatedSource })).toMatchObject({
      appFrame: 4,
      bytes: generatedSource.renderedFrame.bytes,
      contentRevision: 'rev-1',
      cacheRevision: 'rev-1:generated:duplicate:key-1:key-9:4',
    });
    ...
    const matchingPreview = { appFrame: 4, frameIndex: 0, bytes: testWebpBytes('data:preview'), keyId: 'key-4', contentRevision: 'rev-1' };
    ...
    const stalePreview = { appFrame: 4, frameIndex: 0, bytes: testWebpBytes('data:stale'), keyId: 'key-other', contentRevision: 'rev-1' };
    expect(findCachedRotoDisplayFrame(4, {
      getPhysicalRenderSource: () => realSource,
      previewFrames: new Map([[4, stalePreview]]),
      dirtyFrames: new Set([4]),
    })).toMatchObject({ appFrame: 4, keyId: 'key-4', bytes: realSource.renderedFrame.bytes });
    ...
    expect(findCachedRotoDisplayFrame(4, {
      ...
      getFrame: () => frame(4, 'real-key', testWebpBytes('generic')),
    })).toBeNull();
    ...
    expect(findCachedRotoReferenceFrame(4, {
      ...
      getRotoFrame: () => frame(4, 'real-key', testWebpBytes('launch-real')),
      getFrame: () => frame(4, 'real-key', testWebpBytes('generic')),
    })).toBeNull();
  });
  ...
  it('replaces rejected dirty pixels with one explicitly accepted physical base', () => {
    ...
    const accepted = frame(4, 'real-key', testWebpBytes('data:accepted'));
    const syncPending = vi.fn();
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
      getSettingsBackground: () => 'white',
    ...
    expect(engine.setPreviewBaseImageUrl).toHaveBeenCalledWith(getFrameBlobUrl(accepted.bytes), undefined, 4);
  ...
  it('no-ops a late effect-driven reload that would paint a cached PARTIAL over a settled accepted full render (regression-refresh-multi-paint 3rd rejection)', () => {
    ...
    const partial = frame(4, 'real-key', testWebpBytes('data:partial'));
    const setReferenceUrl = vi.fn();
    const setRepaintBaseFrame = vi.fn();
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
  ...
  it('no-ops the pixel-cache fallback retry (replaceDirtyFrame=true) when a completion render already settled for the same frame (regression-refresh-multi-paint 4th rejection)', () => {
    ...
    const partial = frame(20, 'real-key', testWebpBytes('data:partial'));
    const setReferenceUrl = vi.fn();
    const setRepaintBaseFrame = vi.fn();
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
  ...
  it('does not block a NEW stroke repaint on a settled frame: the authoritative accept (explicit generation) still paints the new render (regression-refresh-multi-paint safety case)', () => {
    ...
    const newRender = frame(4, 'real-key', testWebpBytes('data:full-after-new-stroke'));
    const dirtyFrames = new Set([4]);
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
      getSettingsBackground: () => 'white',
    ...
    expect(engine.setPreviewBaseImageUrl, 'the new render must be painted over the old settled base').toHaveBeenCalledWith(getFrameBlobUrl(newRender.bytes), 8, 4);
  ...
  it('still paints a plain refresh for a DIFFERENT appFrame than the settled completion render', () => {
    ...
    const target = frame(4, 'real-key', testWebpBytes('data:frame4'));
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
      getSettingsBackground: () => 'white',
      dirtyFrames: new Set(),
    ...
    expect(engine.setPreviewBaseImageUrl).toHaveBeenCalledWith(getFrameBlobUrl(target.bytes), undefined, 4);
  });
  ...
  it('Layer 2: threads the frame CONTENT token into the engine when no explicit generation is passed', () => {
    ...
    const engine = createEngine();
    const cached = { ...frame(4, 'real-key', testWebpBytes('data:frame4')), contentRevision: 'rev-2' };
    ...
    ).toHaveBeenCalledWith(getFrameBlobUrl(cached.bytes), 42, 4);
  });
  ...
  it('Layer 2: the completion reconcile keeps the reconcile explicit-generation path and always paints', () => {
    ...
    const accepted = { ...frame(4, 'real-key', testWebpBytes('data:new-full')), contentRevision: 'rev-3' };
    const dirtyFrames = new Set([4]);
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
      getSettingsBackground: () => 'white',
    ...
    expect(engine.setPreviewBaseImageUrl).toHaveBeenCalledWith(getFrameBlobUrl(accepted.bytes), 8, 4);
  });

  it('paints an explicit-generation paint (replaceDirtyFrame) even while a settled render is applied', () => {
    const engine = createSettledCompletionEngine(4, 'data:old-full', 7);
    const accepted = frame(4, 'real-key', testWebpBytes('data:new-full'));
    const dirtyFrames = new Set([4]);
    const loader = createRotoReferenceLoader({
      getWorkflowMode: () => 'roto',
      getSettingsBackground: () => 'white',
  ...
  it('returns the accepted (full) frame, not a lingering progressive partial, once the accepted render settles', () => {
    const realSource = {
      ...
      contentRevision: 'rev-1',
      cacheRevision: 'rev-1:real:key-4',
      renderedFrame: { frameIndex: 0, appFrame: 4, bytes: pngDataUrl('full') },
    };
    ...
    const partialPreview = { appFrame: 4, frameIndex: 0, bytes: pngDataUrl('partial'), keyId: 'key-4', contentRevision: 'rev-1' };
    expect(findCachedRotoReferenceFrame(4, {
      getPhysicalRenderSource: () => realSource,
      previewFrames: new Map([[4, partialPreview]]),
      dirtyFrames: new Set(), // accepted render settled → frame no longer dirty
    })).toMatchObject({ appFrame: 4, bytes: pngDataUrl('full') });
  });
