...
import type { PhysicPaintApplyPayload, PhysicPaintApplyResult, PhysicPaintLaunchContext, PhysicPaintPlayRenderOptionsSnapshot, PhysicPaintRotoBackgroundMetadata, PhysicPaintRotoInterpolationMode } from '../../types/physicPaint';
...
export function PhysicsPaintStudio() {
  ...
  const undo = useCallback(() => {
    ...
    if (workflowMode === 'roto' && cachedRotoRepaintBaseFrame?.appFrame === currentFrame) {
      const nextCount = Math.max(0, (liveRotoOverlayActionCountRef.current.get(currentFrame) ?? 0) - 1);
      if (nextCount === 0) {
        liveRotoOverlayActionCountRef.current.delete(currentFrame);
        dirtyRotoFramesRef.current.delete(currentFrame);
        rotoSession.markLiveOverlayEmpty(currentFrame);
        syncPendingRotoFrames();
      } else {
        liveRotoOverlayActionCountRef.current.set(currentFrame, nextCount);
      }
    }
  ...
  function loadCachedPlayPreviewFrame(previewFrame: number): boolean {
    ...
    if (savedPlayCacheDirty) {
      ...
      if (engine && workflowMode === 'play') (engine as PreviewBackgroundEngine).clearPreviewBaseImage();
      return false;
    }
    ...
    if (engine && workflowMode === 'play') {
      (engine as PreviewBackgroundEngine).clearPreviewBaseImage();
  ...
  function findCachedRotoReferenceFrame(appFrame: number, context: PhysicPaintLaunchContext | null = launchContext): RenderedFramePayload | null {
    ...
    return findCachedRotoDisplayFrame(appFrame, context)
  ...
  function findCachedRotoDisplayFrame(appFrame: number, context: PhysicPaintLaunchContext | null = launchContext): RenderedFramePayload | null {
    if (!context) return null;
    const displayCacheFrame = context.cachedRotoFrames?.find((frame) => frame.appFrame === appFrame && (frame.source === 'real-key' || frame.source === 'generated-interpolation'))
      ?? physicPaintStore.getRotoFrame(context.layerId, appFrame);
    if (displayCacheFrame?.source === 'generated-interpolation') return displayCacheFrame;
    const previewFrame = rotoPreviewFramesRef.current.get(appFrame);
    if (previewFrame) return previewFrame;
    if (displayCacheFrame) return displayCacheFrame;
    const confirmedFrame = confirmedCachedRotoFramesRef.current.get(appFrame);
    return confirmedFrame && (confirmedFrame.displayFrame ?? confirmedFrame.appFrame) === appFrame ? confirmedFrame : null;
  }
  ...
  function loadCachedRotoReferenceFrame(
    ...
    setCachedRotoReferenceUrl(null);
    setCachedRotoRepaintBaseFrame(cachedFrame ?? null);
    targetEngine.setBgMode(settings.background);
    targetEngine.clear();
    if (cachedFrame?.dataUrl) {
      targetEngine.setPreviewBaseImageUrl(cachedFrame.dataUrl);
      const wasDirty = dirtyRotoFramesRef.current.delete(appFrame);
      const hadLiveOverlay = liveRotoOverlayActionCountRef.current.delete(appFrame);
      if (wasDirty || hadLiveOverlay) syncPendingRotoFrames();
      setApplyMessage(`Cached key base loaded — visible and non-editable. Add paint to update frame ${appFrame}.`);
    } else {
      targetEngine.clearPreviewBaseImage();
  ...
  const clearCachedRotoReferenceUrl = useCallback(() => {
    setCachedRotoReferenceUrl(null);
  }, []);
  ...
  const clearActiveSource = useCallback(() => {
    ...
    if (workflowMode === 'roto') {
      if (cachedRotoRepaintBaseFrame?.appFrame === currentFrame) {
        ...
        rotoCapturedFramesRef.current.delete(currentFrame);
        liveRotoOverlayActionCountRef.current.delete(currentFrame);
        dirtyRotoFramesRef.current.delete(currentFrame);
        rotoSession.markLiveOverlayEmpty(currentFrame);
        syncPendingRotoFrames();
  ...
  const snapshotCurrentRotoFrame = useCallback(() => {
    ...
    const appFrame = currentFrame;
    if ((cachedRotoReferenceUrl || cachedRotoRepaintBaseFrame?.appFrame === appFrame) && !dirtyRotoFramesRef.current.has(appFrame)) {
  ...
  const flushRotoFrame = useCallback(async (frame: number, options: { force?: boolean; advanceToFrame?: number | null; onPayload?: (payload: PhysicPaintApplyPayload) => void } = {}) => {
    ...
    const flushPromise = (async () => {
      ...
      try {
        ...
        const cachedRepaintBase = cachedRotoRepaintBaseFrame?.appFrame === frame ? cachedRotoRepaintBaseFrame : null;
        if (!capturedFrame && frame !== currentFrame) engine.load(editableState);
        rotoFrameStatesRef.current.set(frame, editableState);
        if (!cachedRepaintBase) setCachedRotoReferenceUrl(null);
        ...
        if (cachedRepaintBase) {
          setApplyMessage(`Merged new paint into frame ${frame}.`);
        } else {
          setApplyMessage('Saving current frame…');
        }
        setLastError(null);
        ...
        setApplyStatus('error');
        if (cachedRepaintBase) {
          setApplyMessage(`Could not merge frame ${frame} — edits are still open. ${detail}`);
        } else {
          setApplyMessage(message);
        }
        setLastError(message);
        if (cachedRepaintBase) setCachedRotoReferenceUrl(null);
        if (cachedRepaintBase) setCachedRotoRepaintBaseFrame(cachedRepaintBase);
        dirtyRotoFramesRef.current.add(frame);
        syncPendingRotoFrames();
        pendingRotoAdvanceRef.current = null;
        saveOnLeaveSourceFrameRef.current = null;
        saveOnLeaveRenderedFrameRef.current = null;
    ...
    return flushPromise;
  }, [actionContext, addEditableRotoFrame, cachedRotoRepaintBaseFrame, canvasHeight, canvasWidth, currentFrame, removeCachedRotoFrameFromLaunchContext, removeEditableRotoFrame, resolveRotoSourceFrameForDisplayFrame, rotoSession, settings, startApplyTimeout, syncPendingRotoFrames, upsertCachedRotoFrameInLaunchContext]);
  ...
  const handleApplyResult = useCallback((detail: PhysicPaintApplyResult | null | undefined) => {
    ...
    if (detail.kind === 'replace-roto-key-frames') {
      ...
      const mergedFrame = pendingCachedRotoMergeFrameRef.current?.frame === frame ? pendingCachedRotoMergeFrameRef.current : null;
      if (mergedFrame) {
        upsertCachedRotoFrameInLaunchContext(mergedFrame.renderedFrame, mergedFrame.backgroundOnly, mergedFrame.onionFrame);
        setCachedRotoReferenceUrl(null);
        setCachedRotoRepaintBaseFrame(mergedFrame.renderedFrame);
        rotoFrameStatesRef.current.delete(frame);
        liveRotoOverlayActionCountRef.current.delete(frame);
        removeEditableRotoFrame(frame);
        pendingCachedRotoMergeFrameRef.current = null;
        if (engine && frame === currentFrame) {
          engine.setBgMode(settings.background);
          engine.clear();
  ...
  const saveRotoFrame = useCallback(async (advanceToFrame: number | null = null, options: { onPayload?: (payload: PhysicPaintApplyPayload) => void } = {}) => {
    ...
    if (cachedRepaintBase && !dirtyRotoFramesRef.current.has(currentFrame) && !snapshotHasLiveOverlay) {
      setApplyStatus('success');
      setApplyMessage(`No new paint to save for frame ${currentFrame}.`);
      return null;
    }
    dirtyRotoFramesRef.current.add(currentFrame);
    syncPendingRotoFrames();
    return flushRotoFrame(currentFrame, { force: true, advanceToFrame, onPayload: options.onPayload });
  }, [cachedRotoRepaintBaseFrame, currentFrame, flushRotoFrame, readyToApply, snapshotCurrentRotoFrame, syncPendingRotoFrames]);
    ...
    <main class="demo-shell">
      ...
      >
        ...
        <PhysicsPaintWorkflowStrip
          ...
          onRotoInterpolationModeChange={(mode) => updateRotoInterpolationSettings({ mode: mode as PhysicPaintRotoInterpolationMode })}
