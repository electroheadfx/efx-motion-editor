...
describe('PhysicsPaintStudio Roto cache relaunch contract', () => {
  it('initializes saved and occupied Roto cells from real cached launch frames after reopen', () => {
    ...
    expect(text).toContain('useState<number[]>(() => getRealCachedRotoDisplayFrameNumbers(launchContext))');
    ...
    expect(text).toContain('setOccupiedRotoFrames(getRealCachedRotoDisplayFrameNumbers(launchContext))');
  ...
  it('shows cached Roto references as an overlay without installing them as the engine paper background', () => {
    ...
    expect(loadBlock).toContain('setCachedRotoReferenceUrl(null)');
    expect(loadBlock).toContain('targetEngine.setBgMode(settings.background)');
    expect(loadBlock).toContain('targetEngine.setPreviewBaseImageUrl(cachedFrame.dataUrl)');
    expect(loadBlock).toContain('targetEngine.clearPreviewBaseImage()');
    ...
    expect(loadBlock).not.toContain("targetEngine.setBgMode(cachedFrame?.dataUrl ? 'transparent' : settings.background)");
    ...
    expect(text).toContain('loadCachedRotoReferenceFrame(hydratedContext.startFrame, readyEngine as PreviewBackgroundEngine, hydratedContext)');
...
describe('PhysicsPaintStudio local Play preview contract', () => {
  ...
  it('36.12 UAT Test 8 wires visible Roto interpolation count through store-owned regeneration and status copy', () => {
    ...
    expect(updateBlock).toContain("mode: 'duplicate'");
    ...
    expect(updateBlock).toContain('settings: refreshedSettings');
    ...
    expect(updateBlock).toContain('physicPaintStore.getRotoInterpolationFailureStatus(launchContext.layerId)');
    expect(updateBlock).toContain('Generated in-betweens could not regenerate. Real keys were kept.');
    expect(updateBlock).toContain('Generated in-betweens on — render-only frames refresh from real keys.');
    expect(updateBlock).toContain('Generated in-betweens on — save at least two real Roto keys.');
    expect(updateBlock).toContain('Generated in-betweens off — real Roto keys only.');
  ...
  it('previews cached Roto playback from cached frames only and avoids mutation paths in the Roto branch', () => {
    ...
    expect(cachedRotoBlock).toContain('physicPaintStore.getRotoFrame(context.layerId, appFrame)');
    expect(cachedRotoBlock).toContain('physicPaintStore.getFrame(context.layerId, appFrame)');
    expect(cachedRotoBlock).toContain('confirmedCachedRotoFramesRef.current.get(appFrame)');
    expect(cachedRotoBlock).toContain('rotoPreviewFramesRef.current.get(appFrame)');
    expect(cachedRotoBlock.indexOf("if (displayCacheFrame?.source === 'generated-interpolation') return displayCacheFrame")).toBeLessThan(cachedRotoBlock.indexOf('rotoPreviewFramesRef.current.get(appFrame)'));
    expect(cachedRotoBlock.indexOf('findCachedRotoDisplayFrame(appFrame, context)')).toBeLessThan(cachedRotoBlock.indexOf('physicPaintStore.getFrame(context.layerId, appFrame)'));
...
describe('PhysicsPaintStudio Roto cache-first autosave contract', () => {
  it('loads cached-only Roto frames as repaintable non-exported references', () => {
    ...
    expect(loadBlock).toContain("context.cachedRotoFrames?.find((frame) => frame.appFrame === appFrame && (frame.source === 'real-key' || frame.source === 'generated-interpolation'))");
    expect(loadBlock).toContain('physicPaintStore.getRotoFrame(context.layerId, appFrame)');
    expect(loadBlock).toContain('physicPaintStore.getFrame(context.layerId, appFrame)');
    expect(loadBlock).toContain("if (displayCacheFrame?.source === 'generated-interpolation') return displayCacheFrame");
    expect(loadBlock.indexOf('findCachedRotoDisplayFrame(appFrame, context)')).toBeLessThan(loadBlock.indexOf('physicPaintStore.getFrame(context.layerId, appFrame)'));
    expect(loadBlock).not.toContain('rotoSession.markCachedBaseLoaded(appFrame)');
    expect(loadBlock).not.toContain('targetEngine.setBackgroundImageUrl(cachedFrame.dataUrl)');
    expect(loadBlock).toContain('targetEngine.resetBackground()');
  ...
  it('keeps beforeunload snapshot-only while explicit Save current remains the cache write path', () => {
    ...
    expect(saveBlock).toContain('return flushRotoFrame(currentFrame, { force: true, advanceToFrame, rotoInterpolationSettings: nextRotoInterpolationSettings, onPayload: options.onPayload })');
  ...
  it('keeps the explicit Save current path on the normal single-frame apply-canvas flow', () => {
    ...
    expect(saveBlock).toContain('flushRotoFrame(currentFrame, { force: true, advanceToFrame, rotoInterpolationSettings: nextRotoInterpolationSettings, onPayload: options.onPayload })');
  ...
  it('D-01 keeps cached repaint base full strength and semantically separate from onion/playback/reference visual treatment', () => {
    ...
    expect(text).toContain('targetEngine.setPreviewBaseImageUrl(cachedFrame.dataUrl)');
    expect(text).toContain('targetEngine.clearPreviewBaseImage()');
  });
  ...
  it('36.11 keeps accepted cached repaint saves in a confirmed in-session source for timeline navigation', () => {
    const text = source();
    const resetBlock = text.slice(text.indexOf('const resetRotoSessionForLaunch = useCallback'), text.indexOf('useEffect(() => {', text.indexOf('const resetRotoSessionForLaunch = useCallback')));
    const findBlock = text.slice(text.indexOf('function findCachedRotoReferenceFrame'), text.indexOf('function findCachedRotoPlaybackFrame'));
    const upsertCallbackBlock = text.slice(text.indexOf('const upsertCachedRotoFrameInLaunchContext = useCallback'), text.indexOf('const clearCachedRotoReferenceUrl = useCallback'));
    const snapshotBlock = text.slice(text.indexOf('const snapshotCurrentRotoFrame = useCallback'), text.indexOf('const startRotoCachedPlayback'));
  ...
  it('disables Roto canvas input while save-on-leave is applying or the current display frame is generated', () => {
    ...
    expect(text).toContain('const currentRotoDisplayFrame = useMemo(() => {');
    expect(text).toContain('return findCachedRotoDisplayFrame(currentFrame);');
    expect(text).toContain("const currentFrameIsGeneratedRoto = currentRotoDisplayFrame?.source === 'generated-interpolation'");
  ...
  it('keeps real-key utility helpers out of the strict Phase 36.3 Roto strip surface', () => {
    ...
    expect(text).toContain('physicPaintStore.setRotoInterpolationSettings(launchContext.layerId, nextSettings)');
