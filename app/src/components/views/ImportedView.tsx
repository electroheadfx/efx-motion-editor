  ...
  return (
    <div class="flex flex-col flex-1 min-w-0 bg-(--color-bg-root)">
      ...
      <div class="flex items-center justify-between h-10 px-4 bg-(--color-bg-toolbar) border-b border-(--color-separator) shrink-0">
        <span class="text-sm font-semibold text-(--color-text-button)">
          {headerText}
        </span>
        <div class="flex items-center gap-2">
          ...
          <button
            class="rounded-[5px] bg-(--color-accent) px-3 py-1.5 hover:bg-(--color-accent-hover) transition-colors"
      ...
      {imageStore.isImporting.value && (
        <div class="flex items-center gap-2 px-4 py-1.5 bg-(--color-bg-selected)">
          <div class="w-3 h-3 border-2 border-(--color-accent) border-t-transparent rounded-full animate-spin" />
          <span class="text-[10px] text-(--color-text-secondary)">Importing...</span>
        </div>
      )}

      {/* Import errors */}
      {imageStore.importErrors.value.length > 0 && (
        <div class="px-4 py-1.5 bg-(--color-error-bg)">
          {imageStore.importErrors.value.map((err, i) => (
            <span key={i} class="text-[9px] text-(--color-error-text) block truncate">{err}</span>
          ))}
        </div>
      )}
      ...
      {isDecodingAudio && (
        <div class="flex items-center gap-2 px-4 py-1.5 bg-(--color-bg-selected)">
          <div class="w-3 h-3 border-2 border-(--color-accent) border-t-transparent rounded-full animate-spin" />
          <span class="text-[10px] text-(--color-text-secondary)">Decoding audio...</span>
        </div>
      )}
