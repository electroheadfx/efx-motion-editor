...
// Loop Clip filmstrip capsule (Phase 43, UI-SPEC Color S1) — hardcoded functional constants
const LOOP_BAND_BASE = 'rgba(255, 255, 255, 0.05)';
const LOOP_BAND_HATCH = 'rgba(255, 255, 255, 0.14)';
const LOOP_GHOST_FILL = 'rgba(255, 255, 255, 0.06)';
const LOOP_GHOST_BORDER = 'rgba(255, 255, 255, 0.24)';
const LOOP_TRUNCATION_COLOR = '#FFB020';
const LOOP_SOURCE_CELL_BORDER = 'rgba(255, 255, 255, 0.22)';
const LOOP_CAPSULE_OUTLINE_IDLE = 'rgba(255, 255, 255, 0.22)';
const LOOP_CAPSULE_OUTLINE_HOVER = 'rgba(255, 255, 255, 0.50)';
const LOOP_BADGE_FILL = 'rgba(13, 13, 13, 0.85)';
const LOOP_BADGE_TEXT = 'rgba(255, 255, 255, 0.85)';
const LOOP_ANCHOR_FLAG_FILL = '#666666';
const LOOP_ANCHOR_FLAG_TEXT = '#E8E8E8';
const LOOP_ERROR_OUTLINE = '#FF4444';
...
export class TimelineRenderer {
  ...
  private drawFxTrack(
    ...
    if (barX + barW >= TRACK_HEADER_WIDTH && barX <= canvasWidth) {
      ...
      if (fxTrack.layerType === 'physic-paint' && fxTrack.loopCapsules?.length) {
        this.drawLoopCapsules(
          ctx,
          fxTrack,
          barX,
          barW,
  ...
  /** Draw Loop Clip filmstrip capsules on a physic-paint FX row (Phase 43, HOLD-06).
   *  The capsule is a pure VIEW of fxTrack.loopCapsules — the 43-02 resolver
   *  derivation projected by frameMap. This method holds NO boundary math: all
   *  extents, ghost-cell grids, diagonal landings, and badge text come from
   *  loopCapsuleGeometry. Canvas paint calls only — zero DOM nodes (D-32). */
  private drawLoopCapsules(
    ctx: CanvasRenderingContext2D,
    fxTrack: FxTrackLayout,
    barX: number,
    barW: number,
    ...
    const capsules = fxTrack.loopCapsules;
    if (!capsules || capsules.length === 0 || barW <= 0) return;
    ...
    const view = { inFrame: fxTrack.inFrame, frameWidth, scrollX, headerWidth: TRACK_HEADER_WIDTH };
    const band = zoomBandForFrameWidth(frameWidth);
    const rangeH = Math.max(10, Math.min(14, Math.round(barH * 0.7)));
    const rangeY = barY + barH - rangeH - 2;
    const centerY = rangeY + rangeH / 2;
    // Visible layer-local frame window — ghost cells/band segments are computed
    // only for this window, never for the whole effective range (D-32).
    const visibleStartFrame = scrollX / frameWidth - fxTrack.inFrame;
    const visibleEndFrame = (scrollX + canvasWidth - TRACK_HEADER_WIDTH) / frameWidth - fxTrack.inFrame;
    ...
    for (const capsule of capsules) {
      const isError = capsule.unresolved !== null;

      // D-22: a zero-effective loop renders the greyed anchor flag pinned at
      // the placement start — never invisible, never a bare gap.
      if (isZeroEffectiveLoop(capsule)) {
        const flag = anchorFlagGeometry(capsule);
        const flagCenterX = loopCapsuleFrameToX(flag.frame, view);
        if (flagCenterX < trackLeft - flag.widthPx || flagCenterX > trackRight + flag.widthPx) continue;
        const flagX = flagCenterX - flag.widthPx / 2;
        const flagY = centerY - flag.heightPx / 2;
        ctx.fillStyle = LOOP_ANCHOR_FLAG_FILL;
        ctx.beginPath();
        ctx.roundRect(flagX, flagY, flag.widthPx, flag.heightPx, flag.heightPx / 2);
        ctx.fill();
        if (isError) {
          ctx.strokeStyle = LOOP_ERROR_OUTLINE;
          ctx.lineWidth = 2;
          ctx.stroke();
        }
        ctx.font = '600 10px system-ui, sans-serif';
        ...
        ctx.fillStyle = LOOP_ANCHOR_FLAG_TEXT;
        ctx.fillText('0f', flagCenterX, centerY);
        ctx.textAlign = 'start';
        continue;
      }

      const capsuleLeftX = loopCapsuleFrameToX(capsule.placementStart, view);
      const capsuleRightX = loopCapsuleFrameToX(capsule.effectiveEnd, view);
      if (capsuleRightX < trackLeft || capsuleLeftX > trackRight) continue;

      // 1. First-cycle cells: downscaled real-key payload thumbnails via
      //    ThumbnailCache + drawImage (D-15). Real-key-backed cells keep the
      //    solid source-cell border (diamonds come from the existing
      //    drawRotoKeyMarkers pass); duplicated-loop first-cycle cells draw the
      //    shared source thumbnails with the dashed linked border and NO diamond.
      for (const cellFrame of firstCycleCellFrames(capsule)) {
        if (cellFrame.frame >= capsule.effectiveEnd) break; // truncated inside the first cycle
        const cell = capsule.firstCycleCells[cellFrame.index];
        const cellX = loopCapsuleFrameToX(cellFrame.frame, view);
        if (cellX + frameWidth < trackLeft || cellX > trackRight) continue;
        const image = cell?.dataUrl && frameWidth >= MIN_FRAME_WIDTH_FOR_THUMB
          ? this.thumbnailCache.get(cell.sourceKeyId, cell.dataUrl)
          : null;
        if (image && image.complete) {
          ctx.drawImage(image, cellX, rangeY, frameWidth, rangeH);
        } else {
          ctx.fillStyle = cellFrame.index % 2 === 0 ? PLACEHOLDER_BG_A : PLACEHOLDER_BG_B;
          ctx.fillRect(cellX, rangeY, frameWidth, rangeH);
        }
        if (cell?.realKeyBacked) {
          ctx.setLineDash([]);
          ctx.strokeStyle = LOOP_SOURCE_CELL_BORDER;
        } else {
          ctx.setLineDash([4, 4]);
          ctx.strokeStyle = LOOP_GHOST_BORDER;
        }
        ctx.lineWidth = 1;
        ctx.strokeRect(cellX + 0.5, rangeY + 0.5, frameWidth - 1, rangeH - 1);
        ctx.setLineDash([]);
      }

      // 2. Repetition region (D-16 zoom-adaptive): ghost linked cells at high
      //    zoom, perforated/hatched band at default zoom, solid band at low zoom.
      const repetitionStartX = loopCapsuleFrameToX(repetitionRegionStartFrame(capsule), view);
      if (capsuleRightX > repetitionStartX) {
        if (band === 'high') {
          for (const cell of visibleGhostCells(capsule, visibleStartFrame, visibleEndFrame)) {
            const ghostX = loopCapsuleFrameToX(cell.startFrame, view);
            const ghostW = (cell.endFrame - cell.startFrame) * frameWidth;
            ctx.fillStyle = LOOP_GHOST_FILL;
            ctx.fillRect(ghostX, rangeY, ghostW, rangeH);
            ctx.setLineDash([4, 4]);
            ctx.strokeStyle = LOOP_GHOST_BORDER;
            ctx.lineWidth = 1;
            ctx.strokeRect(ghostX + 0.5, rangeY + 0.5, ghostW - 1, rangeH - 1);
            ctx.setLineDash([]);
          }
        } else {
          const bandLeft = Math.max(repetitionStartX, trackLeft);
          const bandRight = Math.min(capsuleRightX, trackRight);
          if (bandRight > bandLeft) {
            ctx.fillStyle = LOOP_BAND_BASE;
            ctx.fillRect(bandLeft, rangeY, bandRight - bandLeft, rangeH);
            if (band === 'default') {
              // 45° hatch, 4px period, 1px lines.
              ctx.strokeStyle = LOOP_BAND_HATCH;
              ctx.lineWidth = 1;
              ctx.beginPath();
              for (let hx = Math.floor((bandLeft - rangeH) / 4) * 4; hx < bandRight; hx += 4) {
                ctx.moveTo(hx, rangeY + rangeH);
                ctx.lineTo(hx + rangeH, rangeY);
              }
              ctx.stroke();
            }
          }
      ...
      // 3. Compact math badge (D-19): 16px pill, 8px padding/radius, 4px inset
      //    from the capsule start, truncateText below the 18px label minimum.
      const badgeText = badgeTextForLoop(capsule);
      ctx.font = '600 10px system-ui, sans-serif';
      const badgeX = Math.max(capsuleLeftX + 4, trackLeft + 4);
      const badgeMaxW = Math.min(capsuleRightX, trackRight) - badgeX - 4;
      if (badgeMaxW >= 18) {
        const pillW = Math.min(ctx.measureText(badgeText).width + 16, badgeMaxW);
        const badgeY = Math.min(Math.max(centerY - 8, barY + 1), barY + barH - 17);
        ctx.fillStyle = LOOP_BADGE_FILL;
        ctx.beginPath();
        ctx.roundRect(badgeX, badgeY, pillW, 16, 8);
        ctx.fill();
        ctx.fillStyle = LOOP_BADGE_TEXT;
        ctx.textBaseline = 'middle';
        ctx.fillText(this.truncateText(ctx, badgeText, pillW - 16), badgeX + 8, badgeY + 8);
      }

      // 4. State paint — locked precedence (D-23): error > focus > selected >
      //    truncation diagonal > hover > idle. States change paint only, never
      //    geometry; the diagonal stays visible under focus and selection.
      const isHovered = state?.hoveredLoopClipId === capsule.loopId;
      const isSelected = state?.selectedLoopClipId === capsule.loopId;
      const isFocused = state?.focusedLoopClipId === capsule.loopId;
      const outlineW = capsuleRightX - capsuleLeftX;

      ctx.setLineDash([]);
      ctx.strokeStyle = isHovered ? LOOP_CAPSULE_OUTLINE_HOVER : LOOP_CAPSULE_OUTLINE_IDLE;
      ctx.lineWidth = isHovered ? 1.5 : 1;
      ctx.beginPath();
      ctx.roundRect(capsuleLeftX, rangeY, outlineW, rangeH, rangeH / 2);
      ctx.stroke();

      // D-21 truncation diagonal: #FFB020 1.5px slicing the trailing top-right
      // corner across the whole capsule band, landing per the geometry module.
      const diagonalFrame = truncationDiagonalFrame(capsule, band);
      if (diagonalFrame !== null) {
        const diagonalX = loopCapsuleFrameToX(diagonalFrame, view);
        if (diagonalX >= trackLeft - rangeH && diagonalX <= trackRight + rangeH) {
          ctx.strokeStyle = LOOP_TRUNCATION_COLOR;
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.moveTo(diagonalX, rangeY + rangeH);
          ctx.lineTo(diagonalX + rangeH, rangeY);
          ctx.stroke();
        }
      }
      ...
      if (isFocused) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.roundRect(capsuleLeftX - 2, rangeY - 2, outlineW + 4, rangeH + 4, rangeH / 2 + 2);
        ctx.stroke();
      }
      if (isError) {
        // D-23/D-31: unresolvable source refs — the capsule never silently
        // disappears; the error outline is never replaced by selected/hover.
        ctx.strokeStyle = LOOP_ERROR_OUTLINE;
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.roundRect(capsuleLeftX, rangeY, outlineW, rangeH, rangeH / 2);
