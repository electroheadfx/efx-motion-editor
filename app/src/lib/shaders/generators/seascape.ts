...
const SOURCE = `// "Seascape" by Alexander Alekseev aka TDM - 2014
...
float map(vec3 p, float seaTime) {
    float freq = 0.16; float amp = u_waveHeight; float choppy = u_choppy;
...
vec3 getSeaColor(vec3 p, vec3 n, vec3 l, vec3 eye, vec3 dist) {
    vec3 SEA_BASE = vec3(u_waterR, u_waterG, u_waterB);
    ...
    color += SEA_WATER_COLOR * (p.y-u_waveHeight) * 0.18 * atten;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    float time = iTime * u_speed * 0.3;
    float seaTime = 1.0 + iTime * u_speed * 0.8;
...
export const seascape: ShaderDefinition = {
  id: 'seascape',
  name: 'Seascape',
  category: 'generator',
  description: 'Realistic ocean with raymarched waves, sky reflections, and specular highlights',
  ...
  params: [
    ...
    { key: 'waveHeight', label: 'Wave Height', type: 'float', default: 0.6, min: 0.1, max: 2, step: 0.05 },
    { key: 'choppy', label: 'Choppy', type: 'float', default: 4.0, min: 1, max: 8, step: 0.1 },
    { key: 'waterR', label: 'Water', type: 'float', default: 0.0, min: 0, max: 0.5, step: 0.01, colorGroup: 'water' },
    { key: 'waterG', label: 'Water', type: 'float', default: 0.09, min: 0, max: 0.5, step: 0.01, colorGroup: 'water', hidden: true },
    { key: 'waterB', label: 'Water', type: 'float', default: 0.18, min: 0, max: 0.5, step: 0.01, colorGroup: 'water', hidden: true },
  ],
};
