...
- **Icon contract.** The real EFX icon set is generated from `SPECS/efxmotioneditor-icon.png` via `tauri icon`; `bundle.icon` names exactly the 5 desktop files (`32x32.png`, `128x128.png`, `128x128@2x.png`, `icon.icns`, `icon.ico`). Preflight requires the array to match, every referenced file to exist non-empty, `icon.icns` to carry the `icns` magic bytes, and the canonical SPECS source to be a 1024x1024 RGBA PNG (via `sips`).
