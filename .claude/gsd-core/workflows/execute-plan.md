...
exactly as specified in the canonical `/Users/lmarques/Dev/efx-motion-editor/.claude/gsd-core/references/tdd.md` reference — the
...
If user_setup exists: create `{phase}-USER-SETUP.md` using the template at `/Users/lmarques/Dev/efx-motion-editor/.claude/gsd-core/templates/user-setup.md` (or its `/Users/lmarques/Dev/efx-motion-editor/.claude/gsd-core/templates/user-setup.compact.md` variant — resolve per `/Users/lmarques/Dev/efx-motion-editor/.claude/gsd-core/references/compact-content-gate.md` §"Streams 1b and 4"). Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use the template at `/Users/lmarques/Dev/efx-motion-editor/.claude/gsd-core/templates/summary.md` (or `summary.compact.md` — same `compact-content-gate.md` resolution as the USER-SETUP template above).
